...
# 3. テストプレイ時、メニューの「ゲーム終了」から「シャットダウン」を選択すると
#    画像・音声素材で、テストプレイ中実際に使用された素材がリストアップされたテ
#    キストファイル "UsedGraphics.txt" "UsedAudio.txt" が作成されます。
# 4. テストプレイ時、メニューの「ゲーム終了」から「未使用素材を隔離して終了」を
#    選択すると、 3 のテキストファイルにない画像・音声素材が "UnusedMaterials"
#    フォルダに隔離されます。また 3 のテキストファイルもこのフォルダ内に移動しま
#    す。
#
# ■ 補足
